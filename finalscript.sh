...
var1=$(cat wages.csv | cut -d , -f 3,4 | sed 's/,/ /g' | grep -v "wage" | sort -k 1 -nr | grep -e "^16" | sort -k 2 -nr | tail -n 1) 
var2=$(cat wages.csv | cut -d , -f 3,4 | sed 's/,/ /g' | grep -v "wage" | sort -k 1 -nr | grep -e "^12" | sort -k 2 -nr | tail -n 1)
